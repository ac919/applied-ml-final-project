...
def label_offense_defense_manzone(presnap_df, plays_df):
  ...
  plays_df = plays_df[plays_df['pff_passCoverageType'] != 'Other']
  ...
  merged_df = presnap_df.merge(
      plays_df[['gameId', 'playId', 'possessionTeam', 'defensiveTeam', 'pff_passCoverageType']],
      on=['gameId', 'playId'],
      how='left'
  )
  ...
  merged_df['pff_passCoverageType'] = merged_df['pff_passCoverageType'].map(coverage_mapping)
  merged_df.dropna(subset=['pff_passCoverageType'], inplace=True)
...
def label_offense_defense_formation(presnap_df, plays_df):
  ...
  Parameters:
  presnap_df (pd.DataFrame): DataFrame containing tracking data with 'gameId', 'playId', and 'team'.
...
def prepare_frame_data(df, features, target_column):
    ...
    for _, group in grouped:
        ...
        target_values.append(group[target_column].iloc[0])
